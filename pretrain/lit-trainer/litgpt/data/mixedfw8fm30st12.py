...
@dataclass
class MixedFW8FM30ST12(DataModule):
    """The MixedFW8FM30ST12 data module is composed of a mix of data.
    ...
    def __post_init__(self):
        ...
        self.stackedu = str(self.data_path).rstrip("/") + "/continued_pretrain/stackedu/train"
        self.required_paths = [self.fineweb, self.finemath, self.stackedu]
        ...
        for path in self.required_paths:
            if not path.startswith("s3://") and not Path(path).is_dir():
                raise FileNotFoundError(
                    "The data path for MixedFW8FM30ST12 is expected to be the directory containing these subdirectories:"
                    f" `pretrain/train`, `continued_pretrain/finemath/train`, `continued_pretrain/stackedu/train`."
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        # Total: 50BT = fineweb: 8BT + finemath: 30BT + stackedu: 12BT
        weights = (8, 30, 12)

...
@dataclass
class FineFineWeb(DataModule):
    """The FineFineWeb data module for pretraining."""
    data_path: Union[str, Path] = Path(os.environ['FINEFINEWEB_FOLDER_PATH_PV'])  #! <-- change this path (make sure that there is a 'train' folder under this path)
        ...
        for path in self.required_paths:
            if not path.startswith("s3://") and not Path(path).is_dir():
                raise FileNotFoundError(
                    "The data path for FineFineWeb is expected to be the directory containing these subdirectories:"

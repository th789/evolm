...
@dataclass
class CodePretrain(DataModule):
    """The CodePretrain data module for pretraining."""

    data_path: Union[str, Path] = Path("/home/ubuntu/efs/.ml/big_data/for_projects/overtraining/continued_pretrain/codepretrain") # <-- change this path
        ...
        for path in self.required_paths:
            if not path.startswith("s3://") and not Path(path).is_dir():
                raise FileNotFoundError(
                    "The data path for CodePretrain is expected to be the directory containing these subdirectories:"

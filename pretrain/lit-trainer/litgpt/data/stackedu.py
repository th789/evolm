...
@dataclass
class StackEdu(DataModule):
    """The StackEdu data module for pretraining."""

    data_path: Union[str, Path] = Path("/path/to/continued_pretrain/stackedu") # <-- change this path
        ...
        for path in self.required_paths:
            if not path.startswith("s3://") and not Path(path).is_dir():
                raise FileNotFoundError(
                    "The data path for StackEdu is expected to be the directory containing these subdirectories:"
